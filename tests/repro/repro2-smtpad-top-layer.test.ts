...
test("top-layer-components-top-view", async () => {
  const topLayerPath = path.join(__dirname, "../fixtures/smtpad-top-layer.json")

  const circuitData = fs.readFileSync(topLayerPath, "utf-8")
  ...
  ).toMatchPngSnapshot(import.meta.path, "top-layer-components-top-view")
})
